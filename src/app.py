...
def summarize_observation(obs_data: dict) -> str:
    """Tổng hợp câu trả lời cuối cùng từ Observation khi Agent không tự kết luận bằng văn bản"""
...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    # Ngữ cảnh tích lũy: câu hỏi gốc + các Observation đã thu được qua từng lượt ReAct
    context = user_query
    executed_calls = set()
    last_observation = {}
    ...
    while step < MAX_ITERATIONS:
        ...
        llm_response = provider.generate_with_tools(context, tools_list, system_prompt=REACT_AGENT_SYSTEM_PROMPT)
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            # Chặn lặp vô ích khi Agent đề xuất lại đúng Action đã thực thi
            call_signature = f"{tool_name}:{json.dumps(arguments, ensure_ascii=False, sort_keys=True)}"
            if call_signature in executed_calls:
                print(f"⚠️ [ReAct Loop]: Agent lặp lại Action đã thực thi — dừng vòng lặp và tổng hợp kết quả.")
                break
            ...
            if not obs_data:
                ...
                trace_logs.append({
                    ...
                    "query": user_query,
                    "action_type": "FINAL_ANSWER",
                    "thought": "Không nhận được dữ liệu Observation từ MCP Server.",
                    "output": final_answer,
                    "latency_ms": 10.0
                })
                return trace_logs
            ...
            # Nạp Observation vào ngữ cảnh cho lượt suy luận kế tiếp (bản chất vòng lặp ReAct)
            context = (
                f"{context}\n\n[Observation từ công cụ {tool_name}]: {obs_str}\n"
                f"Dựa trên kết quả trên: nếu yêu cầu của sinh viên chưa hoàn tất, hãy gọi tiếp công cụ phù hợp; "
                f"nếu đã đủ dữ liệu, hãy đưa ra câu trả lời cuối cùng."
            )

    # Vòng lặp kết thúc mà Agent chưa tự chốt bằng văn bản: tổng hợp từ Observation gần nhất
    final_answer = summarize_observation(last_observation) if last_observation else "Chưa thu thập được dữ liệu từ MCP Server."
